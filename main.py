...
from mcp_tools import web_search_tools
...
# === 注册时间工具 ===
@mcp.tool()
def get_current_time(unused_param=None) -> TextContent:
    """获取当前时间格式化后的的日期时间字符串，格式：yyyy年MM月dd日 HH点mm分ss秒 星期，农历"""
    try:
        info = time_tools.get_complete_calendar_info()
        return TextContent(type="text", text=f"今天日期信息为: {str(info["complete_display"])}") 
    except Exception as e:
        return TextContent(type="text", text=f"工具执行错误: {str(e)}") 

@mcp.tool()
def web_search(query: str) -> TextContent:
    """使用网络搜索实时的最新的信息"""
    try:
        info = web_search_tools.search(query)
